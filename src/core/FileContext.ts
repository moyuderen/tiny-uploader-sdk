...
class FileContext {
  ...
  constructor(file: File, uploader: Uploader, defaults: UserFile | null) {
    ...
    this.hasher = this.uploader.hasher as Hashion
    ...
    if (defaults) {
      Object.keys(defaults).forEach((key) => {
        // @ts-ignore
        this[key] = defaults[key as string]
      })
  ...
  generateId() {
    ...
    if (!customGenerateUid || !isFunction(customGenerateUid)) {
      return generateUid()
    } else {
  ...
  async checkRequest() {
    ...
    const statusHandlers = {
      ...
      [CheckStatus.Success]: (data: string) => {
        this.changeStatus(FileStatus.Success)
        updateChunksStatus(ChunkStatus.Success)
        this.url = data
      },
    ...
    try {
      ...
      // 增强错误信息
      const enhancedError: any = new Error(`Check request failed: ${(error as any).message}`)
      enhancedError.originalError = error
      throw enhancedError
    }
  }

  addUploadingChunk(chunk: Chunk) {
    this.uploadingChunks.add(chunk)
  }

  removeUploadingChunk(chunk: Chunk) {
